...
User → FastAPI Backend → Claude (Bedrock) → film script + style + analysis → Build Luma prompt → Luma Ray 2 (Async Invoke via Bedrock) → Output MP4 stored in S3 bucket → Returned JSON + S3 video URL
